...
export const LIBRARY_TYPE_COLORS: Record<LibraryType, { bg: string; text: string; border: string }> = {
  ...
  'white-label':  { bg: 'bg-orange-500/15',    text: 'text-orange-400',  border: 'border-orange-500/30' },
...
const LS_LIBRARIES = 'береста:libraries'
...
function loadLibraries(): Library[] {
  try {
    const raw = localStorage.getItem(LS_LIBRARIES)
